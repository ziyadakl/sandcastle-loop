 ...
 *
 * DUAL TREE — this file is duplicated at `.sandcastle/lib/status/schema.ts`. The
 * two copies MUST stay byte-identical (same convention as `verdicts/schemas.ts`
 * ↔ its `src` twin, see CONTEXT.md). The loop imports the `.sandcastle/lib`
 * copy at runtime; the viewer imports this `src` copy. The zod schema is the
 * single source of truth — `SandcastleStatus` is its inferred type, so a schema
 * edit can never drift from the type consumers use. Bump
 * `STATUS_SCHEMA_VERSION` (and the viewer's tolerance) on any breaking change.
 */
