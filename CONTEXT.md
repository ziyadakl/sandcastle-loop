...
**Drift caveat:** the ported rubric blocks are COPIES of the upstream skills and will drift when those skills change. Each ported block carries a `(ported from … — SYNC PERIODICALLY)` pointer. When the upstream `code-review` / `thermo-review` / `test-driven-development` skills change materially, re-sync these blocks. A dedicated adversarial multi-shard pre-land gate is a deliberate deferred follow-up, gated on verifying nested-sub-agent support (or building it as orchestrator-dispatched review roles).
