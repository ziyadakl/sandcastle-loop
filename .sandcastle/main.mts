...
async function runIssuePipeline(
  ...
  try {
    ...
    if (review.marker === "ALL_CLEAR") {
      ...
      // Under --no-staging (legacy flow), flip label → done immediately.
      // Under staging (default), keep the issue in `in-progress` until the
      // merger lands it on `integration-candidate`; the outer loop will
      // flip it to `merged-to-staging` then (post-fast-forward) to `done`.
