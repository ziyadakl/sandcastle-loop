...
- deleting build/output directories, `node_modules`, lockfiles
...
If the project's install command (`pnpm install`, `npm install`,
`yarn install`, `pip install`, `cargo build`, `bundle install`, etc.)
fails, you do NOT work around it. Specifically forbidden:

- `--force`, `--legacy-peer-deps`, `--no-engine-strict`, `--ignore-scripts`
- deleting the dependency cache and reinstalling to "see if it fixes itself"
- editing lockfiles by hand
- downgrading or upgrading a dep just to make the install pass
- adding override / resolution blocks without spec instruction
...
typecheck/lint (STEP 4), commit a WIP snapshot of your work. This is a
safety net — if a later step (the full test run, an install failure,
...
certification check on `WIP:` commits and treat them as preserved-
progress only.
...
After unit tests are green and BEFORE the full test run, invoke the
`/simplify` skill on the code you changed in this iteration. The skill
reviews changed code for reuse, quality, and efficiency and fixes anything
it finds.

Why here: simplify may edit code. Running it after unit tests confirms you
started from a working baseline; running it before the full test run means
the final test pass was executed against the simplified code, not a
pre-simplified draft.

If `/simplify` makes ANY edits:

- Re-emit `[STEP 4/9]` and re-run typecheck/lint.
