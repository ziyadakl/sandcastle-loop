...
- Run the project's typecheck + tests as you go (auto-detect from
  project files: `pytest` for Python, `pnpm vitest run` / `npm test`
  for JS/TS, `cargo test` for Rust, `go test ./...` for Go, `mix test`
  for Elixir, `bundle exec rspec` for Ruby). The minimal variant has no
  browser stack — do NOT attempt Playwright. The reviewer will re-run
  these after you exit; if they're red when you finish, the fix didn't
  land.
